...
begin
    ...
    if (empleado.apellido <> valoralto) then begin
...
begin
    ...
    while (empleado.apellido <> valoralto) do begin
...
begin
    ...
    while (ref.apellido <> valoralto) do begin
...
procedure leer (var archivo: tipoArchivo; var reg: RegistroEmpleado);
begin
...
var
    empleado: RegistroEmpleado;
begin
    leer (ArchivoEmpleados, empleado);

    while (not eof (ArchivoEmpleados) AND (empleado.apellido <> valoralto)) do begin
        mostrarEmpleado (empleado);
        leer (ArchivoEmpleados, empleado);
    end;
end;
...
procedure borrarTRUNCANDO (var archivo: tipoArchivo);
var 
    reg, last: RegistroEmpleado;
    numero: integer;
begin
    reset (archivo);
    write ('Ingrese codigo a eliminar: '); readln(numero);

    leer (archivo, reg);
    while (numero <> valoralto) do begin
        seek (archivo, filesize(archivo)-1);
        leer (archivo, last);
        seek (archivo, 0);

        while (reg.numero <> numero) do begin
            if (reg.numero = numero) then begin
...
begin
    ...
    7: begin // borrado logico 
        writeln ('Ingrese nombre del archivo: ');
        readln (nombreFisico);
        assign (ArchivoEmpleados, nombreFisico); // asigna

        reset (ArchivoEmpleados); // abre
        borrarReg (ArchivoEmpleados);
        close (ArchivoEmpleados); // cierra
    end;
end;
end.
