...
public class VisitaOslo {
	...
	public List<String> paseoEnBici(Graph<String> lugares, String destino, int maxTiempo, List<String> lugaresRestringidos) {
		...
		if (!lugares.isEmpty()) {
			...
			if (origen != null && dest != null) {
				...
				helper (lugares, origen, visitados, destino, camino, maxTiempo);
			}
		}
	...
	private boolean helper (Graph<String> lugares, Vertex<String> vertex, boolean[] visitados, String destino, List<String> camino, int tiempo) {
		visitados[vertex.getPosition()] = true;
		camino.add(vertex.getData());
		boolean paro = false;
		
		if (vertex.getData().equals(destino)) {
			System.out.println ("Tiempo restante: " + tiempo + " minutos.");
			return true;
		}
		else {
			for (Edge<String> edge: lugares.getEdges(vertex)) {
				...
				if (!visitados[edge.getTarget().getPosition()] && !paro && tiempo - tiempoNecesario >= 0)
					paro = helper (lugares, edge.getTarget(), visitados, destino, camino, tiempo-tiempoNecesario);
			}
		}
